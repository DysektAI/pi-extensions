...
const FINAL_OUTPUT_PREVIEW_CHARS = 6000;
const FINAL_OUTPUT_TOTAL_CHARS = 24000;
...
function truncateText(text: string, maxChars: number): string {
	...
	return `${text.slice(0, maxChars)}\n\n…[truncated ${text.length - maxChars} chars; full output is available in subagent details]`;
}
...
function formatFinalOutputs(results: SingleResult[]): string {
	let remaining = FINAL_OUTPUT_TOTAL_CHARS;
	const sections: string[] = [];
	for (const result of results) {
		const rawOutput = getFinalOutput(result.messages) || result.errorMessage || result.stderr || "(no output)";
		const perResultLimit = Math.min(FINAL_OUTPUT_PREVIEW_CHARS, Math.max(1000, remaining));
		const output = truncateText(rawOutput, perResultLimit);
		sections.push(`${formatResultHeader(result)}:\n\n${output}`);
		remaining -= output.length;
		if (remaining <= 0) {
			sections.push(`…[additional subagent output omitted from tool text; inspect structured details.results for full messages]`);
			break;
		}
	}
...
type DisplayItem =
	...
	| { type: "toolResult"; toolName: string; output: string; isError: boolean };

function getToolResultOutput(msg: Message): string {
	const parts: string[] = [];
	for (const part of msg.content) {
...
function getDisplayItems(messages: Message[]): DisplayItem[] {
	...
	for (const msg of messages) {
		if (msg.role === "assistant") {
			...
			const output = getToolResultOutput(msg);
			if (output) {
				items.push({ type: "toolResult", toolName, output, isError });
			}
		}
	}
	return items;
			...
			const renderDisplayItems = (items: DisplayItem[], limit?: number) => {
				...
				for (const item of toShow) {
					if (item.type === "text") {
						...
						const resultText = preview
							.split("\n")
							.map((line, i) => (i === 0 ? prefix + theme.fg(item.isError ? "error" : "toolOutput", line) : theme.fg("muted", "    ") + theme.fg(item.isError ? "error" : "toolOutput", line)))
							.join("\n");
						text += `${resultText}\n`;
					}
				}
						...
						for (const item of displayItems) {
							if (item.type === "toolCall") {
								...
								for (let i = 0; i < lines.length; i++) {
									const linePrefix = i === 0 ? prefix : theme.fg("muted", "    ");
									container.addChild(
										new Text(
											linePrefix + theme.fg(item.isError ? "error" : "toolOutput", lines[i]),
											0,
											0,
										),
									);
