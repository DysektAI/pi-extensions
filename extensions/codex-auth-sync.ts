 ...
 *   (running any codex command refreshes the file; Pi then pulls it).
 */
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import type { OAuthCredentials, OAuthLoginCallbacks, OAuthProviderInterface } from "@earendil-works/pi-ai/oauth";
import { registerOAuthProvider, loginOpenAICodex } from "@earendil-works/pi-ai/oauth";
...
type PiOAuthCredential = OAuthCredentials & { type: "oauth" | "api_key"; [key: string]: unknown };
...
function codexCredential(): PiOAuthCredential | undefined {
  ...
  if (mode === "apiKey") {
    ...
    // Pi's openai-codex provider is OAuth-shaped; carry the key in `access`.
    return { type: "api_key", access: key, refresh: "", expires: Date.now() + 365 * 24 * 60 * 60 * 1000, authMode: "apiKey" };
  }
...
function isExpired(cred: PiOAuthCredential): boolean {
  // api_key / agentIdentity tokens aren't refreshed via Codex's file.
...
function syncCodexAuthToPi(): string | undefined {
  ...
  // Skip the write if Pi already holds the same (non-stale) access token.
  if (existing && existing.access === cred.access) return undefined;

  piAuth["openai-codex"] = cred;
  saveJson(piPath, piAuth);
  return `Synced ${cred.authMode ?? "chatgpt"} credentials from Codex CLI`;
}

// ============================================================================
// OpenAI Codex OAuth provider — refresh = re-read Codex's file, no network
// ============================================================================
const codexSourcedOAuthProvider: OAuthProviderInterface = {
  id: "openai-codex",
  ...
  // Pi calls this when the stored token is expired. We do NOT hit OpenAI here:
  // doing so would consume the rotating refresh token the Codex CLI owns.
  async refreshToken(_credentials: OAuthCredentials): Promise<OAuthCredentials> {
    ...
    // Persist whatever Codex last wrote (extra fields like type/accountId are fine).
    return cred;
  },
...
export default function (pi: ExtensionAPI) {
  registerOAuthProvider(codexSourcedOAuthProvider);
